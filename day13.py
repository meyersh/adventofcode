...
def repairb(x,y):
    global gridy
    # Look around x,y and interpolate the correct track.
    try:
        if (gridy[y][x-1] in ('-', '+') and gridy[y][x+1] in ('-', '+')
            and gridy[y-1][x] in ('|', '+', '\\', '/') and gridy[y+1][x] in ('|', '+', '\\', '/')):
            return '+'
    except: pass # not an intersection.

    try:
        if (gridy[y][x-1] in ('-', '\\', '/', '+')
            and gridy[y][x+1] in ('-', '\\', '/', '+')):
            return '-'
    except: pass # not a horizontal.

    try:
        if (gridy[y-1][x] in ('|', '\\', '/', '+')
            and gridy[y+1][x] in ('|', '\\', '/', '+')):
            return '|'
    except: pass # not a vertical.

    try:
        if gridy[y][x+1] in ('-', '+') and gridy[y-1][x] in ('|', '+'):
            return '\\'
    except: pass # not Bottom left
    
    try:
        if gridy[y][x-1] in ('-', '+') and gridy[y-1][x] in ('|', '+'):
            return '/'
    except: pass # not Bottom right

    try:
        if gridy[y][x+1] in ('-', '+') and gridy[y+1][x] in ('|', '+'):
            return '/'
    except: pass # not Top left
        
    try:
        if gridy[y][x-1] in ('-', '+') and gridy[y+1][x] in ('|', '+'):
            return '\\'
    except: pass # not Top right.

    # Well... What about the last grid?
    print("Trying to place in from lastgrid -> '%s'" % lastgrid[y][x])
    return lastgrid[y][x]

def repaira(x,y):
    global gridy, lastgrid
    # Look around x,y and interpolate the correct track.
    try:
        if (gridy[y][x-1] in ('-', '+') and gridy[y][x+1] in ('-', '+')
            and gridy[y-1][x] in ('|', '+', '\\', '/') and gridy[y+1][x] in ('|', '+', '\\', '/')):
